...
//! hands it one: a
//! [`CreatedKey`](openrouter_keymaster_core::client::CreatedKey) parsed out of
...
use openrouter_keymaster_core::ids::{Address, OperationId};
use openrouter_keymaster_core::receiver::{
...
fn metadata(hash: &openrouter_keymaster_core::ids::KeyHash) -> DeliveryMetadata {
...
fn a_receiver_is_chosen_by_the_configuration_and_never_by_default() {
    ...
    let scratch = TempDir::new().expect("a temporary directory");
    let configured = openrouter_keymaster_core::config::Receiver::File {
        path: scratch.path().join("jobfeed.key"),
    };
    let openrouter_keymaster_core::config::Receiver::File { path } = &configured else {
...
fn a_configured_command_block_builds_the_command_receiver() {
    let scratch = TempDir::new().expect("a temporary directory");
    let configured = openrouter_keymaster_core::config::Receiver::Command {
