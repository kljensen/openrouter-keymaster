...
response is lost creates a live key that no local record names — an unbounded
spending credential belonging to nobody. A process that dies after parsing the
...
below is persisted durably and atomically before the external action it
...
an unacknowledged key findable later; writing it after the request would leave
exactly the window this ADR exists to close.
...
ambiguous. Receiver invocation is **at-most-once**: an ambiguous
acknowledgement — timeout, signal, lost process status, broken pipe after the
envelope was written — is journaled as `delivery_ambiguous` and never retried
automatically. A receiver may have committed the secret before the
acknowledgement was lost, and re-invoking it could write a stale or duplicate
secret to a live destination.
...
Because each phase is durable before its external action, an interruption at
any point leaves a phase on disk that the next run can read. No phase is
...
disabled, verified, and eventually deleted. **It does not recover the key.**
OpenRouter returns plaintext only in the create response; there is no read
endpoint that returns it, and the hash is an identifier, not a derivation of
the secret. Finding the hash tells Keymaster which object to shut off. It can
never tell Keymaster what the key was. Remediation for any lost plaintext is
always to create a replacement, never to recover the original.

## Consequences

An unacknowledged create is always findable: a durable journal entry with an
operation ID, timestamp, and intended identity exists before any request goes
out, so an orphaned live key can be traced to the attempt that made it.

A key is never delivered before its budget and guardrail are verified, so the
worst case of a failed delivery is a disabled, tracked key rather than an
unrestricted credential in the wild.

Rotation cannot cause an outage. The predecessor stays enabled until an
explicit `retire`, so a replacement that fails at any phase leaves the running
consumer untouched.
...
- **Ambiguity halts the run and requires a human.** Any timeout or 5xx on
  ...
  This is deliberate — the alternative is unbounded orphaned credentials — but
  it is a real cost.
